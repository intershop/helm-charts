...
|<<_example_podAntiAffinity,podAntiAffinity>>|configures the default https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#affinity-and-anti-affinity[pod anti-affinity] for `icm-as-server` pods|yaml object|{optional}|enabled=true, required=false
...
[#_example_podAntiAffinity]
==== podAntiAffinity

By default, anti-affinity is enabled for the `icm-as-server` pods and prefers scheduling pods onto different nodes.
...
The rendered deployment includes:
            ...
            - weight: 100
              podAffinityTerm:
                ...
                labelSelector:
                  matchLabels:
                    app.kubernetes.io/name: icm-as
                    app.kubernetes.io/instance: my-icm-as-appserver
                    app.kubernetes.io/component: appserver
----

Set `required: true` to enforce strict anti-affinity.
...
This changes the rendered deployment to:
