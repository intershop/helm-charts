...
|fileMode|https://www.linuxfoundation.org/blog/blog/classic-sysadmin-understanding-linux-file-permissions[file permissions] of the mounted file|file permissions (octal)|{optional}|-
...
[NOTE]
====
The value for `fileMode` has to use an octal value as used for typical Linux file permissions.
====
...
<4> Set file permissions to `0440` (readable by owner and group, not writable)
