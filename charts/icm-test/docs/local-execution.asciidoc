...
**For replication test suites:** If you are running an icm replication test suite, there will be two different icm instances, both of which need to be rerouted to the correct hostname. In that case, you need to add two lines to the hosts file which look like this:

`<YOUR_IP_ADDRESS> <RELEASE_NAME>-live.test.intershop.com`
`<YOUR_IP_ADDRESS> <RELEASE_NAME>-edit.test.intershop.com`
...
=== 8. Creating the ICMJob custom resource (only necessary for icm-job-tests)

When running a job server test, it is necessary to create the ICMJob custom resource before running the test. It is defined in the icm-job chart and can be created using the following command:
[source,bash]
----
kubectl apply -f charts/icm-job/crds/icm-job-controller.yaml
----

=== 9. Run the `start-test-local.sh` script
