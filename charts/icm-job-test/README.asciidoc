...
=== Initial Setup - ICM System Namespace

Before installing this chart, you must create the `icm-system` namespace, create a Docker Pull Secret in that namespace, and install the `icm-job` chart to it:

[source,bash]
----
kubectl create namespace icm-system
kubectl create secret docker-registry <yourDockerRegistryName> --docker-server=<yourDockerRegistryServer> --docker-username=<yourUsername> --docker-password=<yourPassword> --docker-email=<yourEmail> --namespace icm-system
helm install icm-job . --namespace icm-system
----
